...
Public Query 行为说明：公开问答只走 routed 链路。服务端负责 Router 调用、Specialist 检索、候选页面读取、Specialist LLM 调用、JSON 解析、引用过滤、日志和审查记录；普通问题的语义理解、证据/自答/澄清/拒答选择由 Router 与 Specialist prompt 驱动。候选页面只会来自 public-safe 目录，`wiki/unconfirmed`、`wiki/forbidden`、模板和 outputs 不会作为正式证据传给 LLM。服务端不生成、改写或替换客户可见答案。
...
外部 Public API 只支持非流式 JSON。请求体传 `stream:true` 会返回 `400 STREAM_NOT_SUPPORTED`；后台需要流式、Prompt、模型执行步骤或审计详情时，请调用 Admin Public 审查接口。
...
用途：后台管理员审查/测试外部 Public 问答能力。该接口与外部 Public 共用同一套提示词、检索、意图、安全和基础清洗逻辑，但会返回审计详情，且不会写入真实用户会话日志。

鉴权：无。请将 Admin API 部署在可信网络内，或在反向代理层增加鉴权。

Content-Type：`application/json`

Request Body：同 `POST /api/v1/public/answer`，不需要传 `persist_log` 或 `simulation`；后端会强制按后台审查请求处理。

Response：同外部 Public 响应，并额外返回 `details`。

### POST `/api/v1/admin/public-answer/audit/stream`

用途：后台管理员流式审查/测试外部 Public 问答能力，返回答案增量和完整执行审计事件。
...
Request Body：同 `POST /api/v1/admin/public-answer/audit`。
...
| `prompt` | `data` | `object` | 是 | 本轮 Public Prompt 审计信息。 | `{ "kind": "public_answer" }` |
| `llm_delta` | `data.delta` | `string` | 是 | 模型原始输出增量。 | `"answer"` |
| `llm_reasoning_delta` | `data.delta` | `string` | 是 | 模型推理/过程增量。 | `"正在检索公开证据"` |
| `step_start` | `data` | `object` | 是 | 执行步骤开始事件。 | `{ "name": "检索公开证据" }` |
| `step_finish` | `data` | `object` | 是 | 执行步骤完成事件。 | `{ "name": "检索公开证据" }` |
...
| `result` | `data.details` | `object` | 是 | 完整审计详情。 | `{ "process_summary": "..." }` |
...
用途：为 SaaS 后台总览页提供聚合状态，包含当前模型、模型数量、待审数量、qmd 状态、Wiki Git 状态和 public 日志策略。
...
| `public_answer_log.enabled` | `boolean` | 否 | public 问答日志是否开启。 | `true` |
| `public_answer_log.redact` | `boolean` | 否 | public 问答日志是否脱敏。 | `true` |
| `public_answer_log.retention_days` | `number` | 否 | public 问答日志保留天数。 | `14` |
...
```bash
curl -X POST http://127.0.0.1:9025/api/v1/admin/chat \
  -H 'Content-Type: application/json' \
  -d '{
    "message": "执行一次健康检查",
    "stream": false,
...
```bash
curl -N -X POST http://127.0.0.1:9025/api/v1/admin/chat/stream \
  -H 'Content-Type: application/json' \
  -d '{
    "message": "执行一次健康检查",
    "stream": true,
...
```bash
curl http://127.0.0.1:9025/api/v1/admin/public-intents
```
...
```bash
curl -X PUT http://127.0.0.1:9025/api/v1/admin/public-intents \
  -H 'Content-Type: application/json' \
  -d '{"source":"version: 1\nrules: []\n"}'
```
...
- 终端 AI 客服只使用 `/api/v1/public/answer` 非流式 JSON；不要传 `stream:true`。
- 终端 AI 客服必须自己维护用户会话，并把最近对话作为 `history` 传入。
- 后台需要流式、Prompt、模型执行步骤或 reasoning 时，使用 `/api/v1/admin/public-answer/audit` 或 `/api/v1/admin/public-answer/audit/stream`。
- 不要把 Admin 审查接口暴露给终端客户，因为其中包含管理员执行细节和 reasoning。

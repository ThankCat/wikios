...
func TestPublicSafeThinkingForLogUsesProcessSummary(t *testing.T) {
	...
	got := publicSafeThinkingForLog(resp, "内部推导：1000个静态IP 4折，10元/个/月。")
	if !containsAny(got, "公开问答安全检查") {
		t.Fatalf("expected process summary in log thinking, got %q", got)
	}
}

func TestPublicRawModelOutputLogSummaryOmitsRawJSON(t *testing.T) {
	got := publicRawModelOutputLogSummary(`{"answer":"这个数量可以申请优惠，折后约 10元/个/月。"}`)

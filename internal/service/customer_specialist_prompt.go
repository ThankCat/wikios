...
const (
	...
	customerSpecialistJSONOnlySuffix     = "\n\n完成上文「输出前自检（L4）」后，只返回一个 JSON 对象，不要用代码块包裹最终 JSON，不要解释。`answer` 必须是面向客户的非空字符串，禁止 `\"answer\":\"\"`；`answer` 字段内部可以使用 Markdown；要问客户的话写在 `answer`，不要只写在 `review_question`。`answer` 里禁止出现知识库、资料库、查询资料、系统检索等内部话术，也不要承诺帮客户提交申请或办理后台操作。"
)

...
export function ChatDetailDrawer({ title, open, selected, onClear }: Props) {
  ...
  return (
    <>
      <div className="absolute inset-0 z-20 bg-zinc-950/15 dark:bg-black/45" onClick={onClear} />
      <aside
        className="absolute inset-y-0 right-0 z-30 flex min-h-0 w-[min(980px,92vw)] max-w-[calc(100%-40px)] translate-x-0 flex-col overflow-visible break-words rounded-l-3xl border-l border-border bg-card shadow-lg transition-transform duration-200 [overflow-wrap:anywhere] dark:border-border dark:bg-card dark:shadow-none"
      >
        ...
        <div className="flex min-h-0 flex-1 flex-col">
          {selected ? (
            <div className="relative min-h-0 flex-1">
              <div ref={detailScroll.viewportRef} className="detail-scroll h-full overflow-y-auto px-5 pb-5">
                <MessageDetails
                  ...
                  leadingContent={
                    <section className="min-w-0 rounded-xl bg-muted/40 px-3 py-2 dark:bg-secondary/50">
                      <div className="mb-2 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                        <span className="break-words rounded-full bg-muted px-2 py-1 [overflow-wrap:anywhere] dark:bg-secondary">
                      ...
                      <div className="relative">
                        <div
                          ...
                          className="detail-scroll max-h-28 overflow-x-hidden overflow-y-auto whitespace-pre-wrap break-words pr-2 text-xs leading-5 text-muted-foreground [overflow-wrap:anywhere] dark:text-muted-foreground"
                        >
                          {selected.content}
                        </div>
                        <ScrollJumpControls
            ...
            <div className="detail-scroll min-h-0 flex-1 overflow-y-auto p-5">
              <div className="break-words rounded-lg border border-dashed border-border bg-card/70 p-6 text-sm leading-6 text-muted-foreground [overflow-wrap:anywhere]">
